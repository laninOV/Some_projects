...
constexpr unsigned WINDOW_HEIGHT = 1000;
    ...
    /* В конструкторе создадим в динамической памяти новый объект QBackingStore.
       Qt использует особый механизм управления памятью parent-child.
...
void MainWindow::setAnimating(bool isAnimating)
{
    ...
    /* этот метод вызывается при каждом событии окна */
bool MainWindow::event(QEvent *event)
{
    /* - если мы не обработали событие, мы должны вызвать метод родительского класса:
       QWindow::event(event), иначе механизм обработки событий Qt будет сломан
...
void MainWindow::renderLater()
{
    ...
    /* Метод будет запускать рисование на буфере кадра, очищать буфер,
       а потом готовый буфер выводить на экран. */
void MainWindow::renderNow()
{
    ...
    if (m_isAnimating){
        renderLater();  // планирование перерисовки
    }
}
